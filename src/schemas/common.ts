...
export const cutoffSchema = entity({
  ...
  quota: strNull('Quota. JEE (JoSAA): AI (all India), HS (home state), OS (other state), or GO/JK/LA (Goa, Jammu and Kashmir, Ladakh). NEET (MCC): AI (all-India quota) or SO (AIIMS/JIPMER open seats).'),
  ...
  opening_rank: intNull('Best rank admitted in this round. JEE: category rank for non-general categories, and the PwD rank list for PwD seats. NEET: all-India rank (AIR) in every category.'),
  closing_rank: intNull('Last rank admitted in this round — the number most consumers filter on. Same rank basis as opening_rank.'),
  source: {
    type: 'string',
    description: 'Where this record comes from. "josaa": official JoSAA opening and closing ranks. "mcc_derived": computed from official MCC round-1 allotment results as the best and worst rank allotted to the seat group, since MCC publishes allotments rather than cutoffs.',
  },
...
export const placementSchema = entity({
  ...
  year: intNull('Year the batch graduated: academic year 2023-24 is 2024.'),
  academic_year: strNull('Graduating academic year as NIRF reports it, e.g. "2023-24".'),
  program_or_dept: strNull('Program block the figures cover, e.g. "UG 4-year programs" or "PG 2-year programs".'),
  graduating: intNull('Students graduating in the minimum stipulated time.'),
  placed: intNull('Students placed.'),
  higher_studies: intNull('Students selected for higher studies.'),
  students_placed_pct: numNull('placed as a percentage of graduating.'),
  median_salary: numNull('Median annual salary of placed graduates in INR; null when nobody in the batch was placed.'),
  average_salary: numNull('Not reported by NIRF; always null.'),
